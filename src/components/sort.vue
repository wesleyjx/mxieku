		...
		mounted(){
			...
			this.$http.get('./src/assets/data/brand.json')
			 	.then((response)=>{
				    ...
				    this.msg4=response.data.parts;
				    this.msg5=response.data.hw;
				    this.msg6=response.data.children;
...
<style scoped>
	...
	#box main #menu .current{
		display: block;
	}
	#box main #menu ul span{
		width: 100%;
		color:#999;
		font-size: 1.2em;
		display: block;
	...
	#box main #menu ul li:nth-of-type(3n){
		margin-right: 0px;
	}
	#box main #menu ul li img{
		width: 100%;
	}
	#box main #menu ul li p{
		color:#999;
		text-align: center;
	}
</style>